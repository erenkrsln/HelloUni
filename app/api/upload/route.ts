...
if (process.env.CLOUDFLARE_ACCOUNT_ID && process.env.CLOUDFLARE_R2_ACCESS_KEY_ID && process.env.CLOUDFLARE_R2_SECRET_ACCESS_KEY) {
...
export async function POST(request: NextRequest) {
    try {
        ...
        try {
            ...
            console.warn("Upload route received non-json body; falling back to defaults. Raw body:", txt?.slice?.(0, 200));
            body = { filename: "unknown.bin", contentType: "application/octet-stream", fileSize: undefined };
        }
