...
export function ImageCropModal({
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      // Verarbeitungsstatus bleibt aktiv, bis der Upload (isUploading) abgeschlossen ist
    } catch (error) {
  ...
  if (isTwitter) {
    return (
      <Dialog open={isOpen} onOpenChange={handleCancel}>
        <DialogContent
          hideCloseButton
          className={`p-0 overflow-hidden rounded-2xl border border-gray-200 bg-white text-gray-900 max-w-[560px] ${className ?? ""}`}
        >
          {/* Kopfzeile */}
