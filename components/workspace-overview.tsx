...
import { MessageSquare, ListTodo, Folder, BarChart2, Info, Calendar, Plus, Zap } from "lucide-react";
import Link from "next/link";
import { useState } from "react";

interface WorkspaceOverviewProps {
  workspaceId: string;
  onTabChange?: (tab: "chat" | "tasks" | "files" | "polls" | "events" | "group-info") => void;
}

export function WorkspaceOverview({ workspaceId, onTabChange }: WorkspaceOverviewProps) {
  ...
  const isOwner = currentUser && groupDisplay && (groupDisplay as any).creatorId === currentUser._id;
  const isAdmin = currentUser && groupDisplay && (groupDisplay as any).adminIds?.includes(currentUser._id);
          ...
          {groupDisplay && (
            <>
              ...
              <p className="text-sm text-gray-500 mb-3">
                {groupMembers?.length || 0} members · Private group
              </p>
              ...
              {workspaceGroup?.currentGoal && (
                <div className="text-xs text-gray-500 bg-blue-50 inline-block px-3 py-2 rounded-full mt-2">
                  🎯 {workspaceGroup.currentGoal}
                </div>
              )}
            </>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          >
            ...
            <p className="text-xs text-gray-500">
              ...
              {workspaceGroup?.groupType && ` · ${workspaceGroup.groupType}`}
            </p>
          </div>
        </div>
