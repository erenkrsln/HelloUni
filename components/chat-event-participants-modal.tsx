...
export function ChatEventParticipantsModal({
  isOpen,
  onClose,
  chatEventId,
  slotIndex,
}: ChatEventParticipantsModalProps) {
  ...
  const members = useQuery(
    ...
    chatEvent ? { conversationId: chatEvent.conversationId } : "skip",
  );
  ...
  const yesUsers = members.filter(
    (m) => slotVotes.find((v) => v.userId === m._id)?.vote === "yes",
  );
  const maybeUsers = members.filter(
    (m) => slotVotes.find((v) => v.userId === m._id)?.vote === "maybe",
  );
  const noUsers = members.filter(
    (m) => slotVotes.find((v) => v.userId === m._id)?.vote === "no",
  );
      ...
      <div className="mb-6">
        ...
        <div className="space-y-3">
          {users.map((user) => (
            <div
              key={user._id}
              className="flex items-center gap-3 bg-muted border border-border p-2.5 rounded-xl"
            >
