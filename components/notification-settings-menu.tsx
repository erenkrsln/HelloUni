...
export function NotificationSettingsMenu({ userId }: { userId: Id<"users"> | undefined }) {
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-72 rounded-2xl border border-border bg-card p-4 shadow-xl z-[60]">
          <div className="flex items-center justify-between gap-3">
            ...
            <button
              ...
              className={`relative inline-flex h-[31px] w-[51px] flex-shrink-0 items-center rounded-full transition-colors duration-200 ease-in-out disabled:cursor-not-allowed disabled:opacity-50 ${
                isSubscribed ? "bg-[#34C759]" : "bg-muted"
              }`}
            >
              <span
                className={`inline-block h-[27px] w-[27px] transform rounded-full bg-background shadow-md transition-transform duration-200 ease-in-out ${
